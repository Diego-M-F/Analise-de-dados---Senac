...
try:
    ...
    df_roubo_cargas = pd.read_csv(ENDERECO_DADOS + 'BaseDP_Roubo_Carga.csv', sep=';', encoding='utf-8')
    df_roubo = pd.read_csv(ENDERECO_DADOS + 'BaseDP.csv', sep=';', encoding='utf-8')
    df_roubo = df_roubo[df_roubo['ano'].isin([2022, 2023])]
    df_roubo_cargas = pd.merge(df_roubo, df_roubo_cargas, on='cod_ocorrencia')

    df_roubo_cargas = (
        df_roubo_cargas.groupby('munic')['roubo_carga']
        .sum()
        .reset_index())

    print(df_roubo_cargas)
...
try:
    ...
    print(30 * '-')
    array_roubo_carga = np.array(df_roubo_cargas['roubo_carga'])
    ...
    df_ocorrencia_roubo_menores = df_roubo_cargas[
        df_roubo_cargas['roubo_carga'] < q2]
    df_ocorrencia_roubo_maiores = df_roubo_cargas[
        df_roubo_cargas['roubo_carga'] > q3]
    #   IQR
    iqr = q3 - q1
    ...
    #   ASSIMETRIA
    assimetria = df_roubo_cargas['roubo_carga'].skew()
    #   CURTOSE
    curtose = df_roubo_cargas['roubo_carga'].kurtosis()
    ...
    #   OUTLIERS
    df_roubo_carga_outliers_inferiores = df_roubo_cargas[
        df_roubo_cargas['roubo_carga'] < limite_inferior]
    df_roubo_carga_outliers_superiores = df_roubo_cargas[
        df_roubo_cargas['roubo_carga'] > limite_superior]
...
try:
    ...
    plt.title("Boxplot dos Dados",fontsize=18)

    # POSIÇÃO 02
    # MEDIDAS
    plt.subplot(2, 2, 2)
    plt.title('Medidas Estatísticas',fontsize=10)
    ...
    plt.text(0.5, 0.9, f'Distância Média e Mediana: {distancia:.0f}', fontsize=10)
    ...
    plt.text(0.5, 0.5, f'Distancia da média e Variância: {distancia_var_media:.0f}', fontsize=9)
    ...
    plt.subplot(2, 2, 3)
    plt.title('Concentração das Distribuições',fontsize=10)
    # Histograma
    ...
    if not df_roubo_carga_outliers_superiores.empty:
        dados_superiores = df_roubo_carga_outliers_superiores.sort_values(by='roubo_carga', ascending=True).tail(10)

# Cria o gráfico e guarda as barras
        barras = plt.barh(dados_superiores['munic'], dados_superiores['roubo_carga'], color='green')
# Adiciona rótulos nas barras
        plt.bar_label(barras, fmt='%.0f', label_type='edge', fontsize=6, padding=1)
...
# Diminui o tamanho da fonte dos eixos
        ...
        plt.title('Outliers Superiores',fontsize=10)    
    else:
